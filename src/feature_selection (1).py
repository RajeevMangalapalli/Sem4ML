...
def mrmr_selection(X, y, n_features):
    ...
    for _ in range(n_features):
        ...
        for f in remaining:
            ...
            redundancy = np.mean([mi_pairs[f][s] for s in selected]) if selected else 0.0
...
def forward_selection(X, y, max_features=MAX_FEATURES):
    ...
    while len(selected) < max_features:
        scores = {
            ...
            for f in features if f not in selected
        }
...
def backward_elimination(X, y):
    ...
    while len(selected) > 1:
        scores = {
            f: _score(X, y, [x for x in selected if x != f])
            for f in selected
        }
...
def forward_backward_selection(X, y, max_features=MAX_FEATURES):
    ...
    while True:
        ...
        candidates = {
            f: _score(X, y, selected + [f])
            for f in X.columns if f not in selected and len(selected) < max_features
        }
...
def rank_features(X, y, feature_subset, top_n=3):
    ...
    for f in pool:
        ...
        rows.append({'feature': f, 'scores': s, 'classwise_avg': cw,
...
def plot_rankings(all_rankings, method_names):
    ...
    fig.suptitle('Top-3 feature ranking per method (individual CV accuracy)', fontsize=12)
    plt.tight_layout()
    plt.show()
...
if __name__ == '__main__':
    ...
    best_method = max(results, key=lambda k: (results[k][1]['failure'][0] + results[k][1]['success'][0]) / 2)
